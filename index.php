...
function top($c) {
    ...
    for ($i = 0; $i < 5 ; $i++){ $c["display_plan_" . $i] = ( 100 < $p = intval($c["plan_" . $i . "_price"])) ? "block;" . sprintf("", $c["plan_" . $i . "_price_formatted"] = number_format($p)) : "none";}
            ...
            if (0 < strlen($_POST["password"])) {
                $_SESSION["expired"] =
                    ($_POST["password"] == $_POST["password_confirm"]) ?
                     ( $c["password"]=md5($_POST["password"]) )&0 : exit("your password can not be confirmed.");
            }
